...
blank_trans = Transition(0, torch.zeros(5, 11, 12, dtype=torch.uint8), None, 0, False)
...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    state = state.to(dtype=torch.uint8, device=torch.device('cpu'))  # Only store last frame and discretise to save memory
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    state = transition[self.history - 1].state.to(device=self.device).to(dtype=torch.float32)
    next_state = transition[self.history + self.n - 1].state.to(device=self.device).to(dtype=torch.float32)
  ...
  def __next__(self):
    ...
    #     prev_timestep -= 1
    state = self.transitions.data[self.current_idx].state.to(dtype=torch.float32, device=self.device)  # Agent will turn into batch
    self.current_idx += 1
    return state
